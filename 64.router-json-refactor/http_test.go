...
func TestLeague(t *testing.T) {
	...
	t.Run("it returns the league table as JSON", func(t *testing.T) {
		...
		store := StubPlayerStore{nil, nil, expectLeague,}
		server:= NewPlayerServer(&store)
		...
		actual := getLeagueFromResponse(t, response.Body)
		assertStatus(t, http.StatusOK, response.Code)
		assertLeague(t, expectLeague, actual)
	})
}
...
func assertLeague(t *testing.T, expect, actual []Player) {
	t.Helper()
...
func assertContentType(t *testing.T, response *httptest.ResponseRecorder, expect string) {
	t.Helper()

	if response.Header().Get("content-type") != expect {
		t.Errorf("response did not have content-type of %s, got %v", expect, response.Result().Header)
	}
}
